...
func (r *VEHICLE) InitGuidance(launchProfile string) *[]Pevent {
	...
	if _, err := os.Stat(filepath); err == nil {
		...
		if err == nil {
			...
			if err == nil {
				//				println("cos(incli)=", math.Cos(deg2rad(profile.OrbitInclination)))
				//				println("cos(llatitude)=", math.Cos(deg2rad(profile.LaunchLatitude)))
				...
				//profile.DeltaGamma = profile.BurnoutTime /
